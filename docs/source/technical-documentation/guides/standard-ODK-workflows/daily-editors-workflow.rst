...
The editors workflow is one of the formal `workflows <index.rst>`_ to ensure that the ontology is developed correctly according to ontology engineering principles. There are a few different editors workflows:
...
- components (anything in src/ontology/components), see `here <your-ODK-repository-overview.rst>`_.

2. Imports should not be edited (any edits will be flushed out with the next update). However, refreshing imports is a potentially breaking change - and is discussed `elsewhere <imports-management.rst>`_.
...
In our experience, having a visual git client like `GitHub Desktop <https://desktop.github.com/>`_ or `sourcetree <https://www.sourcetreeapp.com/>`_ is really helpful for this part. In case you prefer the command line:
...
More on how to set this up `here <continuous-integration.rst>`_. Once the pull request is created, the CI will automatically trigger. If all is fine, it will show up green, otherwise red.
